...
function KoreanPromptMockup() {
  return (
    <div className="relative h-full w-full flex items-center justify-center bg-gray-950 p-6">
      {/* Glow */}
      <div
        className="pointer-events-none absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-72 h-72 rounded-full opacity-20 blur-3xl"
        style={{ background: "radial-gradient(circle, #6366f1, transparent)" }}
      />

      <div className="relative z-10 w-full max-w-xs space-y-3">
        {/* Input bubble */}
        <div className="rounded-xl border border-gray-700 bg-gray-900 px-4 py-3 shadow-lg">
        ...
        {/* Translated bubble */}
        <div className="rounded-xl border border-indigo-500/30 bg-indigo-950/40 px-4 py-3 shadow-lg">
          <p className="text-xs text-indigo-400 mb-1.5 font-mono">Auto-translated</p>
        ...
        {/* Result */}
        <div className="rounded-xl border border-green-500/30 bg-green-950/30 px-4 py-3 shadow-lg flex items-center gap-3">
          ...
          <div>
            <p className="text-xs text-green-400 font-mono">이미지 생성 완료</p>
            <p className="text-xs text-green-300/60 mt-0.5">1024 × 1024</p>
          </div>
        </div>
      </div>
    </div>
  ...
  return (
    <section id="features" className="mx-auto max-w-5xl px-6">
      <div className="mb-12 text-center">
        <h2 className="text-2xl font-bold text-white sm:text-3xl">이런 걸 할 수 있어요</h2>
      ...
      <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
        {features.map((f) => (
          <div
            key={f.title}
            className="group overflow-hidden rounded-xl border border-gray-800 bg-gray-900/40 transition-all duration-300 hover:border-gray-700 hover:bg-gray-900/60"
          >
