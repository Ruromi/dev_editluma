    ...
    <div className="mx-auto max-w-6xl px-6 py-10">
      <div className="mb-8 flex flex-col gap-4 lg:flex-row lg:items-end lg:justify-between">
        <div className="space-y-3">
          <span className="inline-flex items-center rounded-full border border-gray-800 bg-gray-900/70 px-3 py-1 text-[11px] font-medium uppercase tracking-[0.18em] text-gray-400">
            안전 결제
          </span>
          <div>
        ...
        <div className="flex items-center gap-3">
          <button
            ...
            className="inline-flex items-center justify-center rounded-xl border border-gray-800 px-4 py-2.5 text-sm font-medium text-gray-300 transition-colors hover:border-gray-700 hover:bg-gray-900 hover:text-white"
          >
            새로고침
          </button>
          {isAuthenticated ? (
            ...
            <Link
              href={loginHref}
              className="inline-flex items-center justify-center rounded-xl border border-indigo-500/30 bg-indigo-500/10 px-4 py-2.5 text-sm font-medium text-indigo-100 transition-colors hover:bg-indigo-500/20"
            >
              로그인하고 결제하기
            </Link>
          )}
      ...
      {isLoading ? (
        ...
        <div className="space-y-6">
          <section className="relative overflow-hidden rounded-[32px] border border-gray-800 bg-gray-950/80 p-6 sm:p-8">
            <div
                ...
                {isAuthenticated ? (
                  <>
                    ...
                    <div className="grid gap-3 sm:grid-cols-2">
                      <div className="rounded-2xl border border-gray-800 bg-gray-900/70 px-4 py-4">
                        <p className="text-[11px] uppercase tracking-[0.16em] text-gray-500">
                          Cost Per Image
                        </p>
                        <p className="mt-2 text-xl font-semibold text-indigo-200">
                          {creditCost} credits
                        </p>
                      </div>
                      <div className="rounded-2xl border border-gray-800 bg-gray-900/70 px-4 py-4">
                  ...
                  <div className="grid gap-4 lg:grid-cols-[1.1fr_0.9fr]">
                    <div className="rounded-[28px] border border-gray-800 bg-gray-900/70 p-5">
                      <p className="text-xs font-medium uppercase tracking-[0.18em] text-gray-500">
                        Pricing Flow
                      </p>
          ...
          <section className="grid gap-4 lg:grid-cols-3">
            {packages.map((pkg) => {
              ...
              return (
                ...
                >
                  {pkg.highlight && (
                    <div className="absolute right-4 top-4 rounded-full border border-indigo-400/30 bg-indigo-500/10 px-3 py-1 text-[11px] font-medium uppercase tracking-[0.16em] text-indigo-200">
                      Recommended
                    </div>
                  )}
                  ...
                  <div className="space-y-4">
                    <div>
                      ...
                      <h2 className="mt-2 text-2xl font-semibold text-white">{pkg.name}</h2>
                    </div>

                    <div className="flex items-end gap-2">
                      <span className="text-4xl font-semibold text-white">{pkg.total_credits}</span>
                    ...
                    {pkg.available ? (
                      ...
                      <button
                        type="button"
                        disabled
                        className="inline-flex w-full items-center justify-center rounded-2xl border border-gray-800 bg-gray-900 px-4 py-3 text-sm font-semibold text-gray-500"
                      >
                        상품 ID 연결 필요
                      </button>
                    )}
          ...
          <section className="rounded-[32px] border border-gray-800 bg-gray-950/80 p-6 sm:p-8">
            <div className="flex flex-col gap-2 sm:flex-row sm:items-end sm:justify-between">
              <div>
                <p className="text-xs font-medium uppercase tracking-[0.18em] text-gray-500">
                  Purchase History
                </p>
                <h2 className="mt-2 text-2xl font-semibold text-white">최근 충전 내역</h2>
              </div>
            ...
            {!isAuthenticated ? (
              <div className="mt-6 rounded-3xl border border-dashed border-gray-800 px-6 py-12 text-center">
                <p className="text-sm text-gray-400">로그인하면 최근 충전 내역과 현재 잔액을 함께 확인할 수 있습니다.</p>
                <Link
                  href={loginHref}
                  className="mt-4 inline-flex items-center justify-center rounded-2xl bg-indigo-600 px-4 py-3 text-sm font-semibold text-white transition-colors hover:bg-indigo-500"
                >
                  로그인하기
                </Link>
              </div>
            ) : history.length === 0 ? (
              <div className="mt-6 rounded-3xl border border-dashed border-gray-800 px-6 py-12 text-center text-sm text-gray-500">
                아직 충전 내역이 없습니다.
              </div>
            ) : (
              <div className="mt-6 overflow-hidden rounded-3xl border border-gray-800">
                <div className="grid grid-cols-[1.4fr_0.9fr_0.8fr_1fr] gap-4 border-b border-gray-800 bg-gray-900/70 px-5 py-3 text-[11px] font-medium uppercase tracking-[0.16em] text-gray-500">
                  <span>Package</span>
                  <span>Credits</span>
                  <span>Amount</span>
                  <span>Created</span>
                ...
                <div className="divide-y divide-gray-800">
                  {history.map((item) => (
                    <div
                      key={item.id}
                      className="grid grid-cols-1 gap-3 px-5 py-4 text-sm text-gray-300 sm:grid-cols-[1.4fr_0.9fr_0.8fr_1fr]"
                    >
                      <div>
                      ...
                      <div className="font-medium text-indigo-200">+{item.credits_added} credits</div>
                      <div>
          ...
          <section className="rounded-[32px] border border-gray-800 bg-gray-950/80 p-6 sm:p-8">
            <div className="flex flex-col gap-2 sm:flex-row sm:items-end sm:justify-between">
              <div>
                ...
                <h2 className="mt-2 text-2xl font-semibold text-white">최근 사용 내역</h2>
              </div>
              <p className="text-sm text-gray-500">
            ...
            {!isAuthenticated ? (
              <div className="mt-6 rounded-3xl border border-dashed border-gray-800 px-6 py-12 text-center">
                <p className="text-sm text-gray-400">로그인하면 최근 사용 내역을 함께 확인할 수 있습니다.</p>
                <Link
                  href={loginHref}
                  className="mt-4 inline-flex items-center justify-center rounded-2xl bg-indigo-600 px-4 py-3 text-sm font-semibold text-white transition-colors hover:bg-indigo-500"
                >
              ...
              <div className="mt-6 overflow-hidden rounded-3xl border border-gray-800">
                <div className="grid grid-cols-[1.6fr_0.7fr_0.8fr_1fr] gap-4 border-b border-gray-800 bg-gray-900/70 px-5 py-3 text-[11px] font-medium uppercase tracking-[0.16em] text-gray-500">
                  <span>Action</span>
                  <span>Used</span>
                  <span>Balance</span>
                  <span>Created</span>
                ...
                <div className="divide-y divide-gray-800">
                  {usageHistory.map((item) => {
                    ...
                    return (
                      <div
                        key={item.id}
                        className="grid grid-cols-1 gap-3 px-5 py-4 text-sm text-gray-300 sm:grid-cols-[1.6fr_0.7fr_0.8fr_1fr]"
                      >
                        <div>
                          <p className="font-medium text-white">
                        ...
                        <div className="font-medium text-rose-300">-{item.credits_used} credits</div>
